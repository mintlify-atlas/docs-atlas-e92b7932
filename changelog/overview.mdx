...
- Added support for `controls` and `autoplay` HTML attributes in the [audio](/dashboard/canopy/edit-audio) and [video](/dashboard/canopy/edit-video) **Canopy** components.
...
- Added `agency` resource in the [API](/api-reference/agencies/overview). You can now manage your agency through the API.

<Badge tone="info">Changed</Badge>

- Changed the `primary_domain_id` response property to `primary_domain` in the [`project` resource](/api-reference/console/projects) to make it consistent with the value passed to the `expand` query parameter.
</Changelog>
...
- Changed [Update domain](/api-reference/console/domains) API to ensure a `project` parameter is required in the body of the request.
</Changelog>
...
- Fixed rendering issues with the [`{% form>` tag](/canvas/tags/form).
</Changelog>
...
- Added [`image_url`](/canvas/filters/overview#image_url) filter to the **Canvas** templating language, to allow you to retrieve an optimised image.
...
- Added new **[`{% form>`](/canvas/tags/form)** **Canvas** tag. Designed to improve the experience of creating forms in your **Canvas** files.
...
- Added Canvas filters to easily create HTML tags. [`image_tag`](/canvas/filters/overview#image_tag), `script_tag` and `stylesheet_tag` have been added.
